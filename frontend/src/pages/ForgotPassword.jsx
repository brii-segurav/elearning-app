...
function ForgotPassword() {
  ...
  const errBox = (msg) => (
    <div style={{ padding: ".75rem 1rem", background: "#FEE2E2", color: "#991B1B",
      ...
      <Icon id="warning" size={16} /><span>{msg}</span>
    </div>
  )
      ...
      <div className="auth-form-side">
        <div className="auth-box fade-in">
          ...
          {step === "email" && (
            <>
              <h2>{t("forgotTitle")}</h2>
              <p className="subtitle">{t("forgotDesc")}</p>
          ...
          {step === "code" && (
            <>
              <h2>{t("enterCode")}</h2>
              {msg && (
                <div style={{ padding: ".75rem 1rem", background: "#D1FAE5", color: "#065F46",
              ...
              <form onSubmit={handleReset}>
                <div className="form-group">
                  <label className="form-label">{t("code6digits")}</label>
                  <input className="input" type="text" placeholder="123456" maxLength={6}
                    value={code} onChange={e => setCode(e.target.value.replace(/\D/g, ""))}
                    required style={{ letterSpacing: ".4rem", fontSize: "1.2rem", textAlign: "center" }} />
                </div>
                <div className="form-group">
              ...
              <p style={{ textAlign: "center", fontSize: ".85rem", marginTop: "1rem" }}>
                <button onClick={() => { setStep("email"); setError(""); setMsg("") }}
          ...
          {step === "done" && (
            <div style={{ textAlign: "center" }}>
              ...
              <h2>{t("passwordUpdated")}</h2>
              <p style={{ marginBottom: "1.5rem" }}>{t("canLoginNow")}</p>
